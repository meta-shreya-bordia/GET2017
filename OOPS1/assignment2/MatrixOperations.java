 ...
 * implements all matrix operations indicated in interface
 * methods:	add, multiply, transpose
 * */
public class MatrixOperations extends Matrix implements InterfaceMatrixOperations {
	...
	/**@Method	addMatrix(Matrix to_add)
	 * add two matrices
	 * matrices must have same row and column size for addition
	...
	/**@Method	multiplyMatrix(Matrix to_multiply)
	 * multiply two matrices
	 * matrices must satisfy m1.column_no = m2.row_no condition to multiply 
	...
	public Matrix multiplyMatrix(Matrix to_multiply){
		...
		for(int row = 0; row< multiplied.row_size; row++){
			for(int column = 0; column< multiplied.column_size; column++){
				...
				multiplied.matrix[row][column] += this.matrix[row][common_index]*to_multiply.matrix[common_index][column];
			}
		}
	 ...
	 * convert row x column matrix to column x row matrix
